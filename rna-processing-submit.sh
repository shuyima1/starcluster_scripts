...
while getopts "m:n:e:g:f:p:x:y:k:h" ARG; do # colons denote which options have an argument after the option
	case "$ARG" in
		m ) MEM=$OPTARG;;
		n ) NTHREADS=$OPTARG;;
		e ) EMAIL=$OPTARG;;
...
echo "Output Location: ./results/${SAMPLEID}/"

## Make the output directory
mkdir -p ./results/${SAMPLEID}/
...
##$STAR --runThreadN $NTHREADS --genomeDir $GENOME_PATH --readFilesIn ${FILE1} ${FILE2} --alignIntronMin 20 --alignIntronMax 500000 --outFilterMismatchNmax 10
echo "STAR complete"
